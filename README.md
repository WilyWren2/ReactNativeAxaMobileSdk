...
Individual callbacks return one or more value.
...
Parameters:
- transactionName is a string
-  callback is a function which expects a response value and an NSError value.
...
```
AXASDK.uploadEvents((response, error) => {
    console.log(` {${response}} ${error}`);
    if (!error) {
        var events=response.CAMDOTotalUploadedEvents;
        var key=response.CAMDOResponseKey;
        console.log(`***uploaded ${events} events (${key}) {${response}} ${error}`);
    } else {
        if (error) {
            // process error message
            console.log(`error: ${error}`)
